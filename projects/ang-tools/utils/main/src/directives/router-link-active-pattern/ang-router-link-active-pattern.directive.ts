...
@Directive({
    selector: '[angRouterLinkActivePattern]'
})
export class AngRouterLinkActivePatternDirective extends BaseDirective implements OnChanges, OnDestroy {

    @Input() angRouterLinkActivePattern: string
    ...
    private isCurrentLinkActive(): boolean {
        ...
        if (this.angRouterLinkActivePattern) {
            const currentUrl = this.router.getCurrentNavigation()
                ? this.router.getCurrentNavigation()?.finalUrl?.toString()
                : this.router.url

            return AngRouterLinkActivePattern.isUrlActive(currentUrl, this.angRouterLinkActivePattern)
        }
